<template>
	<div class="c-application c-telling-avatar-quote">
		<div class="telling-wrapper">
			...
			<CTypography v-if="$slots['content']" :type="isMobile ? 'body2' : 'body1'" color="gray700">
				<slot name="content" />
			</CTypography>
		</div>
		<div class="avatar-wrapper">
			<CAvatar type="image" :src="avatarImage" :size="!isMobile ? 'large' : 'small'" />
			<CTypography v-show="!isMobile" type="caption1" color="gray500" class="avatar-name-wrapper">
				<div>{{ avatarName }}</div>
...
export default {
	...
	components: { CTypography, CAvatar },
};
</script>
