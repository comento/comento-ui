    ...
    template:
        `<Dropdown :max-width="maxWidth" :max-height="maxHeight" :label="label" :full="full" :vertical="vertical">
            <template v-slot:item>
                <SearchInput full :showSearchDropdown.sync="showSearchDropdown" :style="computedSearchInputMaxWidth" />
            </template>
...
<Canvas>
    <Story name="All" height="240px">
        {{
                ...
                <div>
                    <div style="margin-bottom: 20px">
                        <Dropdown max-width="230px" label="searchInput Dropdown">
                            <template v-slot:item>
                                <SearchInput full :showSearchDropdown.sync="showSearchDropdown" style="width: 230px" />
                            </template>
                            <template v-slot:list>
                                <List v-if="showSearchDropdown" spacing>
                                    <template v-for="(item, index) in listItems">
            ...
            `,
            components: { Dropdown, SearchInput, List, ListItem, Divider, Input },
        }}
    </Story>
</Canvas>
