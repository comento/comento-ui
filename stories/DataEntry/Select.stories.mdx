import { ArgsTable, Meta, Story, Canvas } from "@storybook/addon-docs/blocks";
import Select, { selectColors } from "@/src/Components/DataEntry/Select";

<Meta
    title="Data Entry/Dropdown/Select"
    component={ Select }
    argTypes={{
        options: {
            description: "select 리스트",
            defaultValue: ['List1', 'List2', 'List3', 'List4', 'List5'],
            control: { type: "array" },
        },
        placeholder: {
            description: "설명 텍스트",
            control: { type: "text" },
        },
        label: {
            description: "라벨 텍스트",
            control: { type: "text" },
        },
        color: {
            description: "화살표 색상",
            control: {
                type: "select",
                options: selectColors,
            },
        },
        value: {
            description: "셀렉트한 값 (스토리북에서는 동작하지 않음)",
        },
        vertical: {
            description: "방향 (label 값이 있어야 함)",
        },
        maxHeight: {
            description: "리스트 최대 높이",
            defaultValue: "130px"
        },
    }}
/>

export const Default = (args, { argTypes }) => ({
    props: Object.keys(argTypes),
    components: { Select },
    data: () => ({
        listItems: ['List1', 'List2', 'List3', 'List4', 'List5'],
    }),
    template: `
        <div>
            <Select
                v-model="value"
                :options="options"
                :placeholder="placeholder"
                :label="label"
                :vertical="vertical"
                :max-height="maxHeight"
                :color="color"
                :full="full"
            />
        </div>
    `,
});

# Select
**Select** 컴포넌트의 문서입니다.

<Story name="Default" height="150px">
    {Default.bind({})}
</Story>

<ArgsTable story="Default" />

## Stories

### Size
<Canvas>
    <Story name="Size" height="200px">
        {{
            data: () => ({
                select1: '',
                select2: '',
                options: [1, 2],
            }),
            template: `
                <div>
                    <div style="margin-bottom: 10px">
                        <Select v-model="select1" :options="options" />
                    </div>
                    <div>
                        <Select v-model="select2" :options="options" full />
                    </div>
                </div>
        `,
            components: { Select },
        }}
    </Story>
</Canvas>

### With Label
<Canvas>
    <Story name="Direction" height="200px">
        {{
            data: () => ({
                select1: '',
                select2: '',
                options: [1, 2],
            }),
            template: `
                <div>
                    <div style="margin-bottom: 10px">
                        <Select v-model="select1" :options="options" label="Label" />
                    </div>
                    <div>
                        <Select v-model="select2" :options="options" label="Label" :vertical="false" />
                    </div>
                </div>
        `,
            components: { Select },
        }}
    </Story>
</Canvas>
