import { ArgsTable, Meta, Story, Canvas } from '@storybook/addon-docs/blocks';
import CChip, { ChipColors, ChipSizes, ChipTypes, ChipSizesWithCloseButton } from '@/components/components/dataDisplay/chips/CChip.vue';
import CTypography from "@/components/elements/core/typography/CTypography.vue";

<Meta
    title="Data Display/Chip/Chip"
    component={ CChip }
    argTypes={{
        text: {
            description: "Chip 들어갈 텍스트",
            defaultValue: 'Chip',
            control: { type: "text" },
        },
        size: {
            description: "Chip 크기",
            defaultValue: "medium",
            control: {
                type: "select",
                options: ChipSizes,
            },
        },
        type: {
            description: "Chip 타입",
            defaultValue: "fill",
            control: {
                type: "select",
                options: ChipTypes,
            },
        },
        color: {
            description: "Chip 색상",
            control: {
                type: "select",
                options: ChipColors,
            },
        },
        full: {
            description: "Chip 너비 full 활성화",
            defaultValue: false,
            control: { type: "boolean" },
        },
        paddings: {
              description: "패딩",
              table: {
                type: {
                  summary: "Array",
                },
              },
              control: {
                type: "array",
              },
              defaultValue: null,
        },
        transparent: {
            description: "outline transparent",
            defaultValue: false,
            control: { type: "boolean" },
        },
        withCloseButton: {
            description: "close button이 있는지"
        },
        clickable: {
            description: "click 가능한 chip인지"
        },
    }}
/>

export const Basic = (args, { argTypes }) => ({
    props: Object.keys(argTypes),
    components: { CChip },
    template:
        `<CChip :type="type" :size="size" :color="color" :full="full" :paddings="paddings" :transparent="transparent" :with-close-button="withCloseButton" :clickable="clickable">{{text}}</CChip>`,
});

# Chip

**c-chip** 컴포넌트의 문서입니다.

<Story name="Basic" height="100px">
    {Basic.bind({})}
</Story>

<ArgsTable story="Basic" />


## Stories
### All
<Canvas>
    <Story name="All" height="100px">
        {{
            template: `
                <div>
                    <template v-for="(type, index) in ChipTypes">
                        <div class="mb-28">
                            <template v-for="(color, index) in ChipColors">
                                <div class="flex">
                                    <div class="flex-row mr-30 mb-8">
                                        <template v-for="(size, index) in ChipSizes">
                                            <CChip :type="type" :color="color" :size="size" class="mr-8">Chip</CChip>
                                        </template>
                                        <template v-if="type === 'fill'">
                                          <template v-for="(size, index) in ChipSizesWithCloseButton">
                                            <CChip :type="type" :color="color" :size="size" with-close-button class="mr-8" @clickCloseButton="handleClickChipClosebutton" @click="handleClick">Chip</CChip>
                                          </template>
                                        </template>
                                    </div>
                                </div>
                            </template>
                        </div>
                    </template>
                </div>
            `,
            data() {
                return {
                    ChipColors,
                    ChipSizes,
                    ChipTypes,
                    ChipSizesWithCloseButton,
                }
            },
            methods: {
                handleClickChipClosebutton() {
                    alert('clickChipCloseButton')
                },
                handleClick() {
                    console.info('handleclick')
                },
            },
            components: { CChip,CTypography },
        }}
    </Story>
</Canvas>

