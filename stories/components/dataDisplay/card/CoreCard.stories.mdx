...
import CCoreCard from '@/components/components/dataDisplay/card/CCoreCard';
...
export const Default = (args, { argTypes }) => ({
    ...
    template: `
        <CNewGrid fluid>
			<CNewRow :style="isMobile ? 'background: #F4F5F7' : ''">
				<CNewCol :col-sm="12" :col-lg="6" no-gutters>
					<CCoreCard
					...
					>
						<template v-slot:replies>
							<template v-if="repliesTemp" >
								<CProfileWithText v-for="(item, index) in repliesTemp" v-if="index < showRepliesCount" :key="'reply-' + index">
									<template v-slot:avatar>
										<CAvatar type="nickname" :text="item.belong.substr(0, 1)" size="small"/>
									</template>
									<template v-slot:name>
                                        <CTypography type="body2" color="gray850" font-weight="semi-bold" element="span">닉네임</CTypography>
                                        <CTypography type="caption1" font-weight="regular" element="span" color="gray500">{{ item.belong }}</CTypography>
									</template>
									<template v-slot:icon>
										<CIcon name="IconCheckRoundSmallFill" color="gray400"/>
									</template>
    ...
    <Story name="All">
        {{
            ...
            template: `
                <CNewGrid fluid>
                    <CNewRow :style="isMobile ? 'background: #F4F5F7' : ''">
                        <CNewCol :col-sm="12" :col-lg="6" no-gutters v-for="(item, index) in questions" :key="index">
                            <CCoreCard
                            ...
                            >
                                <template v-slot:chips>
                                    <CChipGroup class="mb-8" style="margin-top: -2px">
                                        <CChip size="small" color="primary">조회수 TOP</CChip>
                                        <CChip size="small">이직 고민</CChip>
                                    </CChipGroup>
                                </template>
                                <template v-slot:replies>
                                    <template v-for="(item, index) in item.replies">
                                        <CProfileWithText v-if="index < 2" :key="'reply-' + item.id">
                                            <template v-slot:avatar>
                                                <CAvatar type="nickname" :text="item.belong.substr(0, 1)" size="small"/>
                                            </template>
                                            <template v-slot:name>
