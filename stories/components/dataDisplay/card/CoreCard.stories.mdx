...
import CoreCard from '@/components/components/dataDisplay/card/CoreCard';
							...
							<template v-if="repliesTemp" >
								<ProfileWithText v-for="(item, index) in repliesTemp" v-if="index < showRepliesCount" :key="'reply-' + index">
									...
									<template v-slot:name>
                                        <Typography type="body2" color="gray850" font-weight="bold" element="span">닉네임</Typography>
                                        <Typography type="caption1" font-weight="normal" element="span" color="gray500">{{ item.belong }}</Typography>
									</template>
