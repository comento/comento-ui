			...
			template: `
                <div>
                    ...
                    <Reply :is-editing="isEditing">
						...
						<template v-slot:value>
							<Textarea v-if="isEditing" :value.sync="comment" type="reply" @submit="isEditing = false" />
							<template v-else>
								<span v-html="nl2br(comment)" />
							</template>
						</template>
						<template v-slot:actions>
							<Popover placement="bottom" class="reply-actions-popover">
								<IconButton icon-name="IconThreeDotSmallFill" color="gray400" />
								<template v-slot:popover>
									<List size="large">
										<ListItem size="large" @click="editComment">
											<Typography type="body2" color="gray800" :font-weight="400" class="flex flex-row align-center">
												<Icon name="IconWritingSmallLine" color="gray700" class="mr-10" />
												수정하기
											</Typography>
										</ListItem>
									</List>
